...
from tests.sampleresponse.xenplatform import xenplatform_callback_url_response
from xendit.models import XenPlatform, XenPlatformURLType


# fmt: off
class TestSetCallbackURL(ModelBaseTest):
    @pytest.fixture
    def default_xenplatform_data(self):
        tested_class = XenPlatform
        class_name = "XenPlatform"
        method_name = "set_callback_url"
        ...
        params = (args, kwargs)
        url = "/callback_urls/invoice"
        expected_correct_result = xenplatform_callback_url_response()
    ...
    @pytest.mark.parametrize("mock_correct_response", [xenplatform_callback_url_response()], indirect=True)
    def test_return_xenplatform_callback_url_on_correct_params(
    ...
    @pytest.mark.parametrize("mock_correct_response", [xenplatform_callback_url_response()], indirect=True)
    def test_return_xenplatform_callback_url_on_correct_params_and_global_xendit(
        self, mocker, mock_correct_response, default_xenplatform_data
    ):
