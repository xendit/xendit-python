...
# fmt: off
class TestGetPayout(ModelBaseTest):
    @pytest.fixture
    def default_payout_data(self):
        ...
        params = (args, kwargs)
        url = f"/payouts/{kwargs['id']}"
        expected_correct_result = payout_response()
    ...
    @pytest.mark.parametrize("mock_correct_response", [payout_response()], indirect=True)
