...
from tests.sampleresponse.direct_debit import multi_payment_response
...
class TestGetPaymentStatus(ModelBaseTest):
    ...
    def default_direct_debit_payment_data(self):
        ...
        method_name = "get_payment_status_by_ref_id"
        ...
        params = (args, kwargs)
        url = f"/direct_debits?reference_id={kwargs['reference_id']}"
        expected_correct_result = multi_payment_response()
    ...
    @pytest.mark.parametrize("mock_correct_response", [multi_payment_response()], indirect=True)
